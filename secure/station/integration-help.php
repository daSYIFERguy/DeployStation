	...
	<main class="station-shell">
		...
		<section class="grid-two">
			<article id="github" class="card">
				<h2>GitHub</h2>
				<p><strong>OAuth (recommended):</strong> If the station owner configured a GitHub OAuth App under Admin → GitHub, use <strong>Sign in with GitHub</strong> on User Settings. The callback URL must match exactly what Admin shows. The app requests <code>repo</code>, <code>workflow</code> (needed to push GitHub Actions files under <code>.github/workflows/</code>), and <code>read:user</code>. If you connected before that scope existed, disconnect and authorize again.</p>
