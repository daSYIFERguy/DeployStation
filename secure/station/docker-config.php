          ...
          <span class="docker-status-meta">
            <?php if ($isNginxInfrastructure): ?>
              ...
              <?php if ($stripUpstreamCookies): ?>
                <br><br><strong>Cookie / Authorization stripping</strong> is enabled globally: those browser headers are not forwarded to this container (see Admin → Projects).
              <?php else: ?>
                <br><br>If <code>/p/…</code> fails <strong>only while you are logged into Station</strong>, try Admin → Projects → <strong>Strip browser Cookie + Authorization to Docker upstream</strong> (some apps error on Station’s session cookie or forwarded tokens).
              <?php endif; ?>
            <?php else: ?>
